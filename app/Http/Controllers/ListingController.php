    ...
    public function index(Request $request) {
        ...
        if($request->has('s')) {
            ...
            $listings = $listings->filter(function($listing) use($query) {
                if(Str::contains(strtolower($listing->title), $query)) return true;
                if(Str::contains(strtolower($listing->company), $query)) return true;
                if(Str::contains(strtolower($listing->location), $query)) return true;
                return false;
            });
        }
